...
import { getRuntimeAgentMbts } from './gt3AgentModelCatalog.js';
...
export function issueCaJobTicket(opts) {
  ...
  // MBTS frozen at session birth so builder and evidence passes of one run never
  // straddle two models. GT3-internal: caSessionFromTicket does not forward it.
  const mbts = getRuntimeAgentMbts();
  return {
    run_id: opts.runId,
    mbts_id: mbts.id,
