...
import { buildBudRecord, writeBudAfterSuccessfulRun } from './lexiom13BudPersist.js';
import {
  EXECUTOR_ID,
  BUILDER_TIMEOUT_MS,
...
const PROFILE_TO_PLUGIN = {
  ...
  document_builder: PLUGIN_IDS.DOCUMENT,
  ...
  software_builder: PLUGIN_IDS.SOFTWARE,
  software_coding_builder: PLUGIN_IDS.SOFTWARE
};

const PROFILE_ALIASES = {
  document_builder: 'document_agent',
  software_builder: 'software_coding_agent',
  software_coding_builder: 'software_coding_agent'
};

export const PREPARE_SOURCE_EPHEMERAL = 'ephemeral_osng_envelope';
export const PREPARE_SOURCE_CANON = 'lexiom_yaml_garden';
...
export async function loadAllOsns(staticRoot) {
...
function collectNativeAncestors(osn, byId, seen, results) {
  ...
  for (const parentId of parentIds) {
    ...
    seen.add(parent.id);
    results.push(parent);
    collectNativeAncestors(parent, byId, seen, results);
  }
}

function includeStandardAncestorNodes(osn, byId, seen, results) {
...
/** Cited spec cone for axis-center: node + native descendants + native ancestors + std-ancestor nodes. */
function includeCitedSpecCone(cited, byId, seen, results) {
  if (!cited) {
    return;
  }
  collectDescendants(cited.id, byId, seen, results);
  collectNativeAncestors(cited, byId, seen, results);
  includeStandardAncestorNodes(cited, byId, seen, results);
}

function gatherSubgraph(rootOsn, byId, seen, results) {
  const scope =
    (rootOsn && rootOsn.compilation && rootOsn.compilation.compilation_scope) || 'self_only';

  if (scope === 'self_as_axis_center') {
    collectDescendants(rootOsn.id, byId, seen, results);
    collectNativeAncestors(rootOsn, byId, seen, results);
    ...
    for (const citedId of citedIds) {
      includeCitedSpecCone(byId.get(citedId), byId, seen, results);
    }
    return;
  }
...
export function resolveSubgraphOsns(rootOsn, byId) {
  const results = [];
  const seen = new Set();
  gatherSubgraph(rootOsn, byId, seen, results);
  return results;
}
...
/**
 * Write ephemeral draft OSNs as YAML under builds/.../osng/ for software plugins
 * (document builds use nodes/*.json and do not need these files as sources).
 */
async function snapshotEphemeralOsngIntoBuildDir(outputDirectory, subgraphOsns) {
  const osngDir = path.join(outputDirectory, 'osng');
  await fsp.mkdir(osngDir, { recursive: true });
  const copied = [];
  for (const osn of subgraphOsns) {
    const fileName = `${String(osn.file_name || osn.id).replace(/\.osn$/i, '')}.osn.yaml`;
    const safeName = fileName.replace(/[<>:"|?*\\/]/g, '_');
    const destAbs = path.join(osngDir, safeName);
    await fsp.writeFile(destAbs, serializeLexiom13OsnYaml(osn), 'utf8');
    copied.push({
      id: osn.id,
      snapshot_path: toPosix(path.join('osng', safeName)),
      source_public_path: null
    });
  }
  return {
    osng_dir: 'osng',
...
function ensureEphemeralCompilation(osn) {
  const next = osn && typeof osn === 'object' ? { ...osn } : {};
  const prior = next.compilation && typeof next.compilation === 'object' ? next.compilation : {};
  let profile = String(prior.target_tool_profile || '').trim();
  if (PROFILE_ALIASES[profile]) {
    profile = PROFILE_ALIASES[profile];
  }
  if (!mapTargetToolProfileToPluginId(profile)) {
    profile = 'document_agent';
  }
  next.compilation = {
    ...prior,
    can_be_compilation_root: true,
    compilation_scope: prior.compilation_scope || 'self_only',
    target_tool_profile: profile
  };
  return next;
}

/**
 * Resolve OSNG source for prepare: canon YAML garden, or ephemeral draft envelope.
 * @returns {{ byId: Map, pathById: Map, rootOsn: object, source: string }}
 */
export async function resolvePrepareOsngSource(staticRoot, repoRoot, body = {}) {
  const proposalRunId = String(body.proposal_run_id || '').trim();
  const hasEnvelope =
    body.osng_envelope &&
    typeof body.osng_envelope === 'object' &&
    Array.isArray(body.osng_envelope.nodes);

  if (proposalRunId || hasEnvelope) {
    let envelope = hasEnvelope ? body.osng_envelope : null;
    let intentHint = '';

    if (proposalRunId) {
      if (!/^[a-zA-Z0-9_-]+$/.test(proposalRunId)) {
        throw httpError(400, 'Invalid proposal_run_id');
      }
      const proposeDir = path.join(repoRoot, 'builds', 'lexiom13-propose', proposalRunId);
      const proposalPath = path.join(proposeDir, OSNG_PROPOSAL_PRIMARY);
      let content;
      try {
        content = await fsp.readFile(proposalPath, 'utf8');
      } catch (error) {
        if (error && error.code === 'ENOENT') {
          throw httpError(404, `OSNG proposal not found for proposal_run_id=${proposalRunId}`);
        }
        throw error;
      }
      try {
        intentHint = String(await fsp.readFile(path.join(proposeDir, 'INTENT.md'), 'utf8')).trim();
      } catch {
        intentHint = '';
      }
      envelope = envelopeFromOsngProposalFile(content, intentHint, {
        max_descendants_effective:
          body.max_descendants_effective != null ? Number(body.max_descendants_effective) : 0
      });
    } else {
      const rootId = String(envelope.root_osn_id || '').trim();
      const nodes = (envelope.nodes || []).map((n) =>
        ensureEphemeralCompilation(normalizeDraftOsn(n, intentHint || n?.seed || ''))
      );
      if (!nodes.length) {
        throw httpError(400, 'osng_envelope.nodes must be non-empty');
      }
    ...
    const byId = new Map();
    const pathById = new Map();
    for (const raw of envelope.nodes || []) {
      const node = ensureEphemeralCompilation(
        normalizeDraftOsn(raw, intentHint || raw?.seed || '')
      );
      byId.set(node.id, node);
    }
    const rootId = String(envelope.root_osn_id || '').trim();
    const rootOsn = byId.get(rootId) || [...byId.values()][0];
    if (!rootOsn) {
      throw httpError(400, 'osng_envelope missing root OSN');
    }
    return {
      byId,
      pathById,
      rootOsn,
      source: PREPARE_SOURCE_EPHEMERAL,
      proposal_run_id: proposalRunId || null
    };
  }
  ...
  if (!compilationRootOsnId) {
    throw httpError(
      400,
      'compilation_root_osn_id, osng_envelope, or proposal_run_id is required'
    );
  }
  ...
  return {
    ...
    pathById,
    rootOsn,
    source: PREPARE_SOURCE_CANON,
    proposal_run_id: null
  };
}

export async function prepareLexiom13Build(staticRoot, repoRoot, body = {}) {
  const resolved = await resolvePrepareOsngSource(staticRoot, repoRoot, body);
  const { byId, pathById, rootOsn, source } = resolved;

  if (!(rootOsn.compilation && rootOsn.compilation.can_be_compilation_root)) {
    throw httpError(400, `OSN is not a compilation root: ${rootOsn.id}`);
  }
  ...
  } else if (source === PREPARE_SOURCE_EPHEMERAL) {
    snapshot = await snapshotEphemeralOsngIntoBuildDir(outputDirectory, subgraphOsns);
    snapshot.format = 'osn_yaml';
    await copyOsngBasicsIntoBuildDir(staticRoot, outputDirectory);
  } else {
  ...
  const handoff = {
    ...
    source,
    proposal_run_id: resolved.proposal_run_id || null,
...
export async function packageDocumentBuilderPrompt(staticRoot, handoff, repoRoot) {
  ...
  const lines = [
    ...
    '- `document.md` and `sections/**` must contain **only the finished SUD itself** — the deliverable the root `output_spec` names (poem, brief, book chapter, etc.).',
    ...
    '- Forbidden in outcome bodies: **notes, remarks, commentary, ceremonial notes, explanations about the artifact, process asides, or “how to read this” appendices**. Put those nowhere in `document.md` / `sections/**`.',
    '- If the SUD is a short form (e.g. a poem), emit that form alone — no title essay, no interpretive afterword, no surrounding meta prose.',
...
async function finishBuilderJobFromReport(handoff, sessionId, reportResult) {
  ...
  if (completed) {
    ...
    if (handoff.source === PREPARE_SOURCE_EPHEMERAL) {
      try {
        const primary = primaryArtifactForPlugin(handoff.plugin_id);
        const content = await fsp.readFile(path.join(outDir, primary), 'utf8');
        if (String(content).trim()) {
          budRecord = buildBudRecord(handoff, { completedAt: new Date().toISOString() });
        }
      } catch {
        /* primary may be missing on failure path */
      }
      budWritten = false;
      budDetailSuffix =
        ' Bud skipped (ephemeral OSNG; no Lexiom YAML canonize).';
    } else {
      try {
...
async function collectHostEvidenceSpansAfterBuilder(handoff, builderSessionId, builderResult) {
  ...
  // Evidence runs on the MBTS frozen at this run's ticket, not the live choice:
  // one run, one model across builder and evidence.
  const frozenRun = getAgentRun(handoff.run_id);
  const collected = await collectEvidenceByQuoteSpans(handoff, {
    model: frozenRun?.mbts_id || null
  });
  return {
...
export async function runLexiom13Build(staticRoot, repoRoot, body = {}) {
  ...
  const running = {
    ...
    // mbts_id is GT3-internal: the CA must not be able to read or echo it.
