...
/** Meta notes / remarks about the artifact — not part of the finished SUD body. */
const DOCUMENT_META_APPENDIX_RE =
  /(^|\n)\s{0,3}#{1,6}\s*(ceremonial\s+)?(note|notes|remarks?|commentary|meta[- ]?note|explanation|about\s+this\s+(poem|hiku|haiku|document|artifact|piece|work))\b/i;
...
export async function validatePrimaryAfterSync(outDir, pluginId, opts = {}) {
  ...
  if (pluginId === 'lexiom13.osng_proposer') {
    let parsed;
    try {
      ...
      return {
        ok: false,
        reason: 'primary_not_json',
        detail: 'OSNG_PROPOSAL.json must be valid JSON'
      };
    }
    const hasNodes = parsed && typeof parsed === 'object' && Array.isArray(parsed.nodes);
    const looksLikeOsn =
      parsed && typeof parsed === 'object' && typeof parsed.id === 'string' && parsed.id.trim();
    if (!hasNodes && !looksLikeOsn) {
      return {
        ok: false,
        reason: 'osng_proposal_shape',
        detail:
          'OSNG_PROPOSAL.json must be { root_osn_id, nodes: [...] } or a single OSN object with id'
      };
    }
    if (hasNodes && parsed.nodes.length < 1) {
      return {
        ok: false,
        reason: 'osng_proposal_empty',
        detail: 'OSNG_PROPOSAL.json nodes[] must be non-empty'
      };
    }
    return { ok: true, primary, bytes: Buffer.byteLength(content, 'utf8') };
  }
  if (pluginId === 'lexiom13.document_builder') {
    ...
    if (DOCUMENT_META_APPENDIX_RE.test(content)) {
      return {
        ok: false,
        reason: 'primary_dirty',
        detail:
          'document.md failed outcome cleanliness (§3.5): notes/remarks/commentary about the artifact must not appear in the SUD body'
      };
    }
    ...
    if (plan?.section_files?.length) {
      ...
      for (const section of plan.section_files) {
        ...
        if (DOCUMENT_META_APPENDIX_RE.test(sectionContent)) {
          return {
            ok: false,
            reason: 'section_dirty',
            detail: `Section has notes/remarks about the artifact (not SUD-only): ${section.path}`
          };
        }
