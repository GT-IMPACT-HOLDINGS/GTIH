 ...
 * Model (MBTS): chosen by the GT3 admin, never by the agent. The CA's placeholder
 * model id is always overwritten here; no request header can select a model.
 */
...
import {
  ...
  MBTS_FAMILY_ANTHROPIC,
  MBTS_FAMILY_OPENAI_REASONING,
  resolveAgentMbts
} from './gt3AgentModelCatalog.js';
import {
  ATTRIBUTION_LANE_BUILD,
  openRouterAttributionHeaders
} from './gt3OpenRouterAttribution.js';

export { DEFAULT_AGENT_OPENROUTER_MODEL };
...
export async function handleAgentChatCompletions(req, res, deps) {
  ...
  const mbts =
    deps.agentMbts ||
    resolveAgentMbts(deps.agentModel) ||
    resolveAgentMbts(DEFAULT_AGENT_OPENROUTER_MODEL);
  const model = mbts.openrouter_slug;
  ...
  const baseMeta = {
    ...
    model,
    mbts_id: mbts.id,
    mbts_family: mbts.family,
    reasoning_effort: mbts.reasoning_effort || null,
    tenant,
    track,
    stream: wantStream
  };
  ...
  const anthropicCacheMarkup =
    isPromptCachingEnabled(deps) && mbts.family === MBTS_FAMILY_ANTHROPIC;
  applyOptionalPromptCaching(outbound, {
    enabled: anthropicCacheMarkup,
    stickyKey: cacheSticky,
    phase: buildPhase,
    runId
  });
  applyProviderShape(outbound, mbts);
  const cacheRouting = applyCacheRouting(outbound, {
    mbts,
    sessionId: runId,
    stickyKey: cacheSticky
  });
  ...
  baseMeta.prompt_cache_enabled = anthropicCacheMarkup;
  baseMeta.prompt_cache_mode = anthropicCacheMarkup
    ? 'anthropic_explicit'
    : mbts.family === MBTS_FAMILY_OPENAI_REASONING
      ? 'openai_automatic'
      : 'provider_automatic';
  baseMeta.session_id = cacheRouting.session_id;
  baseMeta.prompt_cache_key = cacheRouting.prompt_cache_key;
  ...
  if (baseMeta.session_id) {
    // Body session_id already wins; the header also groups the run in the
    // OpenRouter Sessions view for non-chat surfaces.
    orHeaders['X-Session-Id'] = baseMeta.session_id;
  }
...
const OPENROUTER_SESSION_ID_MAX = 256;

/**
 * Cache routing keys OpenRouter actually understands.
 *
 * `session_id` pins the whole run to one provider endpoint, which keeps a warm
 * cache reachable and activates stickiness on the first success instead of only
 * after an observed hit. It is family-agnostic and does not depend on
 * GT3_AGENT_PROMPT_CACHE: it is routing, not cache semantics, and a miss costs
 * nothing. `prompt_cache_key` is OpenAI's own cache partition, so it carries the
 * finer phase-level key for reasoning models whose caching is automatic.
 *
 * @param {object} outbound
 * @param {{ mbts: { family: string }, sessionId: string | null, stickyKey: string | null }} opts
 * @returns {{ session_id: string | null, prompt_cache_key: string | null }}
 */
export function applyCacheRouting(outbound, { mbts, sessionId, stickyKey }) {
  const result = { session_id: null, prompt_cache_key: null };
  if (!outbound) return result;

  const session = boundedKey(sessionId || stickyKey);
  if (session && outbound.session_id === undefined) {
    outbound.session_id = session;
    result.session_id = session;
  } else if (typeof outbound.session_id === 'string') {
    result.session_id = outbound.session_id;
  }

  if (mbts && mbts.family === MBTS_FAMILY_OPENAI_REASONING) {
    const cacheKey = boundedKey(stickyKey || sessionId);
    if (cacheKey && outbound.prompt_cache_key === undefined) {
      outbound.prompt_cache_key = cacheKey;
      result.prompt_cache_key = cacheKey;
    } else if (typeof outbound.prompt_cache_key === 'string') {
      result.prompt_cache_key = outbound.prompt_cache_key;
    }
  }
...
function boundedKey(value) {
  ...
  if (!s) return null;
  return s.length > OPENROUTER_SESSION_ID_MAX
    ? s.slice(0, OPENROUTER_SESSION_ID_MAX)
    : s;
}

/**
 * Normalize the outbound packet to the shape the chosen MBTS family accepts.
 * The CA speaks one dialect (Anthropic-friendly OpenAI-compat); GT3 translates.
 * @param {object} outbound
 * @param {{ family: string, reasoning_effort: string | null }} mbts
 */
export function applyProviderShape(outbound, mbts) {
  if (!outbound || !mbts || mbts.family !== MBTS_FAMILY_OPENAI_REASONING) {
    return outbound;
  }
  if (mbts.reasoning_effort && outbound.reasoning === undefined) {
    outbound.reasoning = { effort: mbts.reasoning_effort };
  }
  // Reasoning models reject non-default sampling controls.
  delete outbound.temperature;
  delete outbound.top_p;
  if (
    outbound.max_tokens !== undefined &&
    outbound.max_completion_tokens === undefined
  ) {
    outbound.max_completion_tokens = outbound.max_tokens;
  }
  delete outbound.max_tokens;
  return outbound;
}
