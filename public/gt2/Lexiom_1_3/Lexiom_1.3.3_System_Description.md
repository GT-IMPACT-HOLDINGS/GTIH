...
**Compilation scopes (runtime):** names are literal. `self_and_approved_descendants` is the clicked node and native descendants only (a ProductLexiom or BrandLexiom click is a fast subtree compile). `self_only` is that node only. `self_plus_parent_context` adds direct native parents. There is no silent graph-root-parent or standard-ancestor prefix.

**Realization-plane axis compiles (runtime):** RealizeProductLexiom and RealizeBrandLexiom are native children of a Realization trunk (`Realization.a1000200.osn`). They share **reciprocal** `standard_ancestor_osn_ids` with ProductLexiom and BrandLexiom. They declare `compilation_scope: self_as_axis_center`. Prepare/run treats the Realize OSN as axis: native ancestors (the trunk) and native descendants; each cited spec is that spec + its native descendants + its native ancestors (parents only) + that spec’s standard-ancestor nodes (visited-set on the reciprocal pair). A RealizeProductLexiom compile therefore includes ProductLexiom’s subtree, GT Philosophy, AccessControl, the axis, and the Realization trunk — not BrandLexiom. RealizeBrandLexiom is the symmetric Brand cone + GT Philosophy + axis + trunk. The Realization trunk is not a compilation root.
...
The left-corner full-graph toggle opens a cabinet-wide exposition with two kinds: **Side view** (indented native trees) and **Top view** / OSNG Garden (radial plants at graph roots and Product/Brand compilation trunks, colored per plane on a green→purple spectrum; solid native arrows including short stems between trunks; dashed cross-tree `standard_ancestor` arrows; plant centers keep packed radii from overlapping; origin-leaf labels follow zoom LOD — see [`Lexiom_1_3_OSNG_Garden_UX_Spec_1_0.md`](Lexiom_1_3_OSNG_Garden_UX_Spec_1_0.md)).
